...
Model weights are large and have to live somewhere. Inference often wants a GPU, which the container needs explicit access to. The data our server collects, like users and request logs, needs to be stored somewhere that does not get wiped out when a container is removed.
...
The steps are documented, versioned, and reproducible. The file itself _is_ the documentation, and we can commit it to Git like any other source file. Anyone with the Dockerfile can rebuild the same image with `docker build`, the same way anyone with the recipe can cook the same dish. Unlike a recipe, the rebuilt image is truly identical, not just similar.
...
CMD ["python", "hello.py"]
```
This Dockerfile starts from the official slim Python 3.13 image, sets the working directory to `/app`, copies `hello.py` from the host into the working directory, and runs `python hello.py` as the default command when a container is started from this image.

We build the image with `docker build`, pointing it at the directory containing the Dockerfile, which is called the **build context**:
```bash
...
- `RUN <command>` executes a shell command during the build, in the most recent layer. Typical uses are installing system packages with `RUN apt-get install ...` or Python dependencies with `RUN pip install ...`.
...
- `ENTRYPOINT ["program"]` sets a fixed command that always runs, with `CMD` providing default arguments. It is useful for images that wrap one specific program. We will not need it for our server.
...
Recall from [Module B.2](@/ai-system/container-fundamentals/index.md#layered-filesystems) that a Dockerfile maps closely to the layers of the resulting image, with roughly one layer per instruction.
...
So we want layers that change rarely above layers that change often. For example, `pip install` for dependencies that we do not touch every day should come above our own application code.
...
> - [**Buildx**](https://docs.docker.com/reference/cli/docker/buildx/) is Docker's modern builder, which adds features like building for multiple CPU architectures, such as an `amd64` server and an `arm64` Raspberry Pi, from the same Dockerfile.
...
This brings us to a recurring design choice when containerizing an AI server. We need to decide which files should live inside the image as layers, and which should be mounted in from the outside as volumes.
...
docker run -d --name ai-server \
    ...
    my-ai-server:1.0
```
We expose a `DATABASE_URL` environment variable that the server reads to find the database file. This requires a small change to `db.py`'s `create_engine` call.
...
The toolkit's docs cover Linux, Docker Desktop, and WSL. Once it is installed on the host, we can run containers with the `--gpus` flag:
```bash
docker run -d --name ai-server \
    --gpus all \
    -p 8000:8000 \
...
1. In the directory containing your Module A.4 server, write a `Dockerfile` along the lines of the one in this module. It should have a slim Python base, dependency install before copying code, an `EXPOSE` for the server's port, and a `CMD` that starts uvicorn bound to all interfaces.
...
2. Mount a host directory as a volume for the SQLite database so the users and request logs survive container removal. Run the server and register a user or log a request, then stop and remove the container, start a new container with the same volume, and verify that the data is still there.
...
4. Push the image to a registry of your choice, with Docker Hub or GHCR being the easiest. Pull it from a different machine if you have one, and confirm that a single `docker run` is enough to bring up a working AI API server with no other setup.
