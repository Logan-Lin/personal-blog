...
And add the new system and Home Manager entries to the `flake.nix`.
...
For example, if I want to introduce a desktop environment to make Steam Deck my Linux PC, all I have to do is to include either the self-contained Hyprland module or the GNOME module in my Nix config repo, and declare the desktop session name as `jovian.steam.desktopSession = "hyprland"` or as `"gnome"`.
